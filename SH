--==================================================
-- 😈 CH PC - Trigger + Macro + ESP
-- TRIGGER | MACRO | ESP | MENU
-- Macro: 2 saca arma → dispara → 2 guarda arma
--==================================================
...
local macroDelay = 0.08
local lastMacroShot = 0
local lastEquipTime = 0
local equipCooldown = 0.35
local gunKey = Enum.KeyCode.Two -- sacar y guardar con 2
...
menu.Size = UDim2.fromOffset(360, 460)
...
subtitle.Text = "TRIGGER | MACRO | ESP | MENU"
...
local function createPage()
	...
	lay:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		p.CanvasSize = UDim2.fromOffset(0, lay.AbsoluteContentSize.Y + 12)
	end)
...
local triggerTab = makeTab("TRIGGER", 0)
local macroTab = makeTab("MACRO", 0.25)
local espTab = makeTab("ESP", 0.5)
local configTab = makeTab("MENU", 0.75)
...
triggerTab.MouseButton1Click:Connect(function() showPage(triggerPage, triggerTab) end)
macroTab.MouseButton1Click:Connect(function() showPage(macroPage, macroTab) end)
espTab.MouseButton1Click:Connect(function() showPage(espPage, espTab) end)
configTab.MouseButton1Click:Connect(function() showPage(menuPage, configTab) end)
...
local triggerbotButton = makeButton(triggerPage, "🔫  Triggerbot  •  OFF")
local delaySlider = createSlider(triggerPage, "Velocidad (1=rápido)  ", 1, 50, 5)
delaySlider.onChanged = function(v) triggerbotDelay = v / 100 end
local friendsButton = makeButton(triggerPage, "👥  Ignorar amigos  •  ON")
local teamButton = makeButton(triggerPage, "🟢  Ignorar equipo  •  ON")

makeSection(macroPage, "TRIGGER CON MACRO")
local macroButton = makeButton(macroPage, "⚡  Macro Trigger  •  OFF")
local macroDelaySlider = createSlider(macroPage, "Delay disparo  ", 1, 50, 8)
macroDelaySlider.onChanged = function(v) macroDelay = v / 100 end
local infoLabel = Instance.new("TextLabel")
infoLabel.Size = UDim2.new(1, 0, 0, 50)
infoLabel.BackgroundTransparency = 1
infoLabel.Text = "1) Mira en el enemigo\n2) Tecla 2 saca el arma\n3) Dispara\n4) Tecla 2 guarda el arma"
infoLabel.TextColor3 = Theme.SubText
infoLabel.Font = Enum.Font.Gotham
infoLabel.TextSize = 11
infoLabel.TextWrapped = true
infoLabel.TextXAlignment = Enum.TextXAlignment.Left
infoLabel.Parent = macroPage
...
local espDistSlider = createSlider(espPage, "Distancia ESP  ", 100, 2000, 1000)
espDistSlider.onChanged = function(v) espMaxDistance = v end

makeSection(menuPage, "MENU")
local keyButton = makeButton(menuPage, "⌨  Tecla  •  " .. MenuKey.Name)
local closeForever = makeButton(menuPage, "🔒  Cerrar para siempre")
...
local function setMenuVisible(state, showBall)
	...
	menu.Visible = state
	floatingButton.Visible = (not state) and (showBall == true)
end

closeButton.MouseButton1Click:Connect(function() setMenuVisible(false, true) end)
floatingButton.MouseButton1Click:Connect(function() setMenuVisible(true, false) end)

local function isFriend(plr)
	if not ignoreFriends or not plr then return false end
...
local function isEnemyExactlyUnderCrosshair()
	...
	if not shouldTarget(player) then return false end
	if not target:IsDescendantOf(character) then return false end
	return true
end
...
local function doMacroTrigger()
	if not macroEnabled then return end
	if not isEnemyExactlyUnderCrosshair() then return end

	local now = os.clock()

	-- 1) Sacar arma (2)
	if not hasGunEquipped() then
		if now - lastEquipTime >= equipCooldown then
			lastEquipTime = now
			pressKey(gunKey)
		end
		return
	...
	-- 2) Disparar
	if now - lastMacroShot < macroDelay then return end
	lastMacroShot = now
	clickMouse()

	-- 3) Guardar arma (2 otra vez)
...
local function createESP(plr)
	if not shouldTarget(plr) then return end
	removeESP(plr)
	...
	bill.Adornee = root
	bill.Size = UDim2.fromOffset(160, 55)
...
macroButton.MouseButton1Click:Connect(function()
	...
	macroButton.Text = macroEnabled and "⚡  Macro Trigger  •  ON" or "⚡  Macro Trigger  •  OFF"
end)

espButton.MouseButton1Click:Connect(function()
	espEnabled = not espEnabled
	espButton.Text = espEnabled and "👁  ESP  •  ON" or "👁  ESP  •  OFF"
	if not espEnabled then for plr in pairs(ESP) do removeESP(plr) end end
end)
...
UserInputService.InputBegan:Connect(function(input, processed)
	if waitingMenuKey and input.UserInputType == Enum.UserInputType.Keyboard then
		if input.KeyCode ~= Enum.KeyCode.Unknown then
			...
			keyButton.Text = "⌨  Tecla  •  " .. MenuKey.Name
		end
		return
	end
	if processed then return end
	if input.KeyCode == MenuKey then
		if menu.Visible then setMenuVisible(false, false) else setMenuVisible(true, false) end
	end
end)
...
print("😈 CH PC | MACRO: 2 saca → dispara → 2 guarda")
